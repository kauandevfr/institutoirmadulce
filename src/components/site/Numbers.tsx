const stats = [
  { k: "+19", v: "anos de atividades sociais" },
...
export function Numbers() {
  return (
    <section id="numeros" className="py-24 md:py-32">
      <div className="container-tight">
        <div className="rounded-[2rem] bg-[var(--gradient-hero)] text-primary-foreground p-10 md:p-16 shadow-elegant relative overflow-hidden">
          ...
          <div className="relative">
            <span className="text-xs font-semibold tracking-[0.2em] uppercase text-white/70">Impacto</span>
            ...
            <dl className="mt-12 grid grid-cols-2 lg:grid-cols-4 gap-8">
              {stats.map((s) => (
                <div key={s.v} className="border-t border-white/20 pt-5">
                  <dt className="font-display text-5xl md:text-6xl font-semibold tracking-tight">{s.k}</dt>
                  <dd className="mt-3 text-sm text-white/80 leading-relaxed">{s.v}</dd>
                </div>
              ))}
            </dl>
          </div>
